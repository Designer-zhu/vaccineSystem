...
public class UserServlet extends BasicServlet {
    ...
    public void register(final HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(flag){
            ...
            String emailMess = "恭喜您注册成功，这是一封激活邮件，请点击<a href='http://localhost:8080/userServlet?method=active&identity="+user.getIdentity()+"&user_id="+user_id+"&code="+code+"'>"+code+"</a>激活";
            try {
    ...
    public void active(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        boolean flag = service.activeState(code,identity);

        //根据user_id查询user对象
        String user_id = request.getParameter("user_id");
        User user = service.viewOneBvUser_id(user_id, identity);
