            ...
            table.on('tool(test)', function(obj){
                ...
                function agree(data) {
                    ...
                    $(function () {
                        $.ajax({
                            type:"post",
                            url:"/recordServlet?method=viewVaccine&v_name="+data.vaccineName+"&v_spec="+data.vaccineSpec,
                                ...
                                layer.open({
                                    ...
                                    success: function (layero, index) {//页面弹出成功触发
                                        ...
                                        form.on("submit(submitFormFilter)", function (d) {
                                            $.post("/recordServlet?method=addNew&id="+data.id, d.field, function (rs) {
                                                //业务正常
                                                if (rs.code == 200) {
                                                    layer.msg("下发成功");
                ...
                function refByV_id(data) {
                    layer.confirm("确定拒绝下发吗？", function (index) {
                        var id = data.id;
                        $.post("/recordServlet?method=refuse&id="+id, function (rs) {
                            //判断业务响应码
                            if (rs.code == 200) {
                                layer.msg("拒绝成功");
